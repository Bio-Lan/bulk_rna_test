...
class MultiqcModule(BaseMultiqcModule):
    def __init__(self):
        ...
        stat_data,sample_list = self.parse_json(ASSAY, "stats")
        well_data = self.parse_well(ASSAY, "well_count")
        ...
        for s in sample_list:
            self.add_section(name = s, anchor = f'{s}_well',helptext=helptext,plot = self.well_table(well_data[s],s))
    ...
    def parse_json(self, assay, seg):
        ...
        data_dict_order = defaultdict(dict)
        n = 0
        sample_list = []
        for f in self.find_log_files(f"{assay}/{seg}"):
            ...
            if parsed_data is not None:
                ...
                if s_name not in sample_list:
                    sample_list.append(s_name)
        
        sample_list = sorted(sample_list)
        for s in sample_list:
            data_dict_order[s] = data_dict[s]
        
        data_dict_order = self.ignore_samples(data_dict_order)
        log.info(f"Found {n} {assay} {seg} reports")
        # Write parsed report data to a file
        self.write_data_file(data_dict_order, f"multiqc_{assay}_{seg}")
        return data_dict_order,sample_list
    
    def parse_well(self, assay, seg):
        data_dict = defaultdict(dict)
        n = 0
        for f in self.find_log_files(f"{assay}/{seg}"):
            log.debug(f"Found file: {f['fn']}")
    ...
    def well_table(self,well_data,sample):
        table_config = {"col1_header":"Well","id":f"{sample}_Well_table"}
        headers = {
            "UMI": {
                "namespace": f"{sample}",
                "title": "UMI",
